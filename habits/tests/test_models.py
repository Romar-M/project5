...
def test_related_habit_must_be_pleasant():
    ...
    pleasant = Habit.objects.create(user=user, place="дом", time="09:00:00", action="чай", is_pleasant=True, duration=30)
    non_pleasant = Habit.objects.create(user=user, place="офис", time="12:00:00", action="обед", is_pleasant=False, duration=45)
    with pytest.raises(ValidationError):
